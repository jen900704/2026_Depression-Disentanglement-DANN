...
架構說明：
  ...
  - Scenario B：speaker map 從 TEST_CSV 建立（38 位 target）
    train 151 位路人 speaker_label=-1，不參與 L_spk
"""
...
class XLSR_eGeMaps_DANN(Wav2Vec2PreTrainedModel):
    def __init__(self, config, egemaps_dim: int = EGEMAPS_DIM):
        ...
        self.down_proj = nn.Sequential(
            nn.Linear(combined_dim, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(0.3),
        )
        self.dep_classifier = nn.Linear(256, config.num_labels)
        self.spk_classifier = nn.Linear(256, getattr(config, "num_speakers", 151))
        self._alpha = 0.0
        self.init_weights()
    ...
    def forward(self, input_values, attention_mask=None, egemaps_feat=None,
                labels=None, speaker_labels=None,
        ...
        if speaker_labels is not None:
            mask = speaker_labels >= 0  # ✅ 過濾 test 陌生人（-1）
            if mask.sum() > 0:
...
def load_audio_dataset(csv_path: str, speaker_to_idx: dict = None, is_train: bool = True):
    ...
    if is_train and speaker_to_idx is None:
        # Scenario B：speaker map 從 TEST_CSV 建立（38 位 target speakers）
    ...
    for _, row in df.iterrows():
        ...
        records.append({
            ...
            "speaker_labels": speaker_to_idx.get(spk_str, -1),  # ✅ 陌生人 → -1
        })
...
if __name__ == "__main__":
    ...
    print("   Spk Map：從 TEST_CSV 建立（38 位 target）")
    print("=" * 60)
    ...
    for run_i in range(1, TOTAL_RUNS + 1):
        ...
        training_args = TrainingArguments(
            ...
            report_to="none",
            remove_unused_columns=False,  # ✅ 防止 Trainer 刪掉 speaker_labels / egemaps_feat
        )
        ...
        try:
            ...
            if "out of memory" in str(e):
                print("⚠️ OOM，跳過此 run"); torch.cuda.empty_cache(); continue
            raise e
        ...
        pth_path = os.path.join(OUTPUT_DIR, f"xlsr_egemaps_dann_B_shared_encoder_run_{run_i}.pth")
    ...
    if all_results:
        print(f"\n{'='*60}\n📈 Scenario B XLS-R+eGeMAPS+DANN — {TOTAL_RUNS} 次彙總\n{'='*60}")
