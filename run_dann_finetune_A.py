"""
File — DANN + Fine-tune (Scenario A) [Linear Probing Family]
============================================================
架構說明：
  - Wav2Vec2 CNN：凍結
  - Wav2Vec2 Transformer：可訓練 (Fine-tune)
  - Pooling：Mean Pooling (取最後一層 hidden state 平均)
  - Classifier：簡單線性分類器 (Linear Probing style)
...
OUTPUT_DIR = "./output_dann_finetune_A_linear"

SEED             = 103
TOTAL_RUNS       = 5
NUM_EPOCHS       = 10
LEARNING_RATE    = 1e-5  # Fine-tune 建議用較小 LR
BATCH_SIZE       = 4
GRAD_ACCUM       = 2
EVAL_STEPS       = 10
SAVE_STEPS       = 10
...
class Wav2Vec2_DANN_FT(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        ...
        # === Fine-tune 設定 ===
        # 1. 凍結 CNN (Feature Extractor)
        self.wav2vec2.feature_extractor._freeze_parameters()
        # 2. Transformer 保持可訓練 (Fine-tune)
        
        # === Linear Probing Head (Simple Linear) ===
