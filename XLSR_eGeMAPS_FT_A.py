"""
XLS-R + eGeMAPS（無 DANN，Fine-tune Transformer，Scenario A）
============================================================
架構說明：
  - XLS-R CNN feature extractor：凍結
  - XLS-R Transformer encoder：可訓練（Fine-tune）
...
修正重點：
  1. BatchNorm1d -> LayerNorm (防止 BS=1 崩潰)
  2. Batch Size 4 -> 2 (防止 XLS-R OOM)
  3. F1 Score -> binary
"""
...
OUTPUT_DIR  = "./output_xlsr_egemaps_ft_A"
EGEMAPS_DIM = 88
...
BATCH_SIZE       = 2      # 修正：降為 2 以防 XLS-R OOM
GRAD_ACCUM       = 4      # 修正：總 Batch = 8
EVAL_STEPS       = 200
SAVE_STEPS       = 200
LOGGING_STEPS    = 10
SAVE_TOTAL_LIMIT = 2
...
# ============================================================
#  模型定義（無 DANN，Fine-tune Transformer）
# ============================================================
class XLSR_eGeMaps_FT(Wav2Vec2PreTrainedModel):
    """
    XLS-R + eGeMAPS，Fine-tune Transformer，無 DANN
    """
    def __init__(self, config, egemaps_dim: int = EGEMAPS_DIM):
        ...
        self.down_proj = nn.Sequential(
            ...
            # 【關鍵修正】 改用 LayerNorm，Batch Size=1 也不會死
            nn.LayerNorm(256),
            nn.ReLU(),
            nn.Dropout(0.3),
        )
...
def compute_metrics(p: EvalPrediction):
    ...
    return {
        ...
        # 修正：統一用 binary (macro 對二元分類來說較少用)
...
def full_evaluation(trainer, test_dataset, output_dir, run_i):
    ...
    plt.title(f"ROC - XLS-R+eGeMAPS FT Scenario A Run {run_i}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 XLS-R + eGeMAPS（無 DANN，Fine-tune Transformer）— Scenario A")
    print(f"   CNN：凍結 | Transformer：可訓練 | eGeMAPS：{EGEMAPS_DIM} 維")
    print("=" * 60)
    ...
    for run_i in range(1, TOTAL_RUNS + 1):
        ...
        model  = XLSR_eGeMaps_FT.from_pretrained(MODEL_NAME, config=config)

        frozen    = sum(1 for p in model.wav2vec2.feature_extractor.parameters() if not p.requires_grad)
        trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"❄️  CNN 凍結: {frozen} 個參數組")
        ...
        training_args = TrainingArguments(gradient_checkpointing=True, 
            ...
            label_names=["labels"],
            # 移除 drop_last=True，因為有了 LayerNorm，我們不需要丟掉數據
            seed=run_seed,
            data_seed=run_seed,
    ...
    if all_results:
        print(f"\n{'='*60}\n📈 Scenario A XLS-R+eGeMAPS FT — {TOTAL_RUNS} 次彙總\n{'='*60}")
