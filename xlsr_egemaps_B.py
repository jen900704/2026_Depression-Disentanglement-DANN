"""
XLS-R + eGeMAPS（無 DANN，無 fine-tune，Scenario B）
=====================================================
架構說明：
...
OUTPUT_DIR  = "./output_xlsr_egemaps_B"
EGEMAPS_DIM = 88
...
def full_evaluation(trainer, test_dataset, output_dir, run_i):
    ...
    plt.title(f"ROC - XLS-R+eGeMAPS Scenario B Run {run_i}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 XLS-R + eGeMAPS（無 DANN）— Scenario B")
    ...
    if all_results:
        print(f"\n{'='*60}\n📈 Scenario B XLS-R+eGeMAPS — {TOTAL_RUNS} 次彙總\n{'='*60}")
