"""
XLS-R + eGeMAPS（無 DANN，Fine-tune Transformer，Scenario B）
============================================================
架構說明：
...
OUTPUT_DIR  = "./output_xlsr_egemaps_ft_B"
EGEMAPS_DIM = 88
...
def full_evaluation(trainer, test_dataset, output_dir, run_i):
    ...
    plt.title(f"ROC - XLS-R+eGeMAPS FT Scenario B Run {run_i}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 XLS-R + eGeMAPS（無 DANN，Fine-tune Transformer）— Scenario B")
    ...
    for run_i in range(1, TOTAL_RUNS + 1):
        ...
        training_args = TrainingArguments(gradient_checkpointing=True, 
            ...
            label_names=["labels"],
            # 移除 drop_last=True，LayerNorm 已經解決了這個問題
            seed=run_seed,
            data_seed=run_seed,
    ...
    if all_results:
        print(f"\n{'='*60}\n📈 Scenario B XLS-R+eGeMAPS FT — {TOTAL_RUNS} 次彙總\n{'='*60}")
